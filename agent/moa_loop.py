...
def _run_reference(
    ...
    *,
    ...
    try:
        # Per-slot max_tokens overrides the preset-level value so individual
        # reference models can have independent output caps (e.g. a verbose
        # model capped at 600 tokens while a terse one gets 2000). The slot
        # value is coerced from the string stored in moa_config; fall back to
        # the preset-level max_tokens when absent or zero.
        slot_max_tokens = max_tokens
        raw_slot_mt = slot.get("max_tokens")
        if raw_slot_mt is not None:
            try:
                _mt = int(raw_slot_mt)
                if _mt > 0:
                    slot_max_tokens = _mt
