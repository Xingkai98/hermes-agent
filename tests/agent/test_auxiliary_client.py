...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        "provider,model,base_url,expected_key,expected_value",
        [
            # GitHub Copilot + GPT-5* → max_completion_tokens
            ("copilot", "gpt-5.4", "https://api.githubcopilot.com", "max_completion_tokens", 1234),
            ("copilot", "gpt-5.5", "https://api.githubcopilot.com", "max_completion_tokens", 1234),
            # Direct OpenAI with GPT-5 → max_completion_tokens (base_url host detection)
            ("custom", "gpt-5", "https://api.openai.com/v1", "max_completion_tokens", 1234),
            # OpenRouter — max_tokens (provider-unaware, no OPENROUTER_API_KEY in test env)
            ("openrouter", "anthropic/claude-sonnet-4.6", "https://openrouter.ai/api/v1", "max_tokens", 1234),
            # Nous Portal — max_tokens
            ("nous", "hermes-4", "https://inference-api.nousresearch.com/v1", "max_tokens", 1234),
            # Custom localhost — max_tokens
            ("custom", "qwen", "http://localhost:8080/v1", "max_tokens", 1234),
            # Zhipu AI — max_tokens
            ("zai", "glm-4v-flash", "https://open.bigmodel.cn/api/paas/v4", "max_tokens", 1234),
        ],
    )
    def test_forwards_max_tokens_for_non_anthropic_providers(self, provider, model, base_url, expected_key, expected_value):
        ...
        # Non-Anthropic/NIM providers now forward max_tokens via
        # auxiliary_max_tokens_param() so auxiliary callers (title generation,
        # MoA references, etc.) respect the caller-specified cap.
