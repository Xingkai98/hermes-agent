...
def _clean_slot(slot: Any) -> dict[str, str] | None:
    ...
    if provider.lower() == "moa":
        return None
    slot_dict: dict[str, str] = {"provider": provider, "model": model}
    # Preserve per-slot max_tokens when present so reference models can set
    # individual output caps that override the preset-level reference_max_tokens.
    slot_max_tokens = _coerce_int(slot.get("max_tokens"), 0)
    if slot_max_tokens > 0:
        slot_dict["max_tokens"] = str(slot_max_tokens)
    return slot_dict
